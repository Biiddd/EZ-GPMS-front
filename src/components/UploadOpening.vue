...
const handleSubmit = async () => {
  ...
  try {
    // 上传校外毕设申请
    if (outSchool) {
      const response0 = await http.post('/stu/upload/outSchool', formData0.value, {
        headers: {
    ...
    // 上传开题报告
    const response1 = await http.post('/stu/upload/start', formData1.value, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    });
    ...
    // 上传外文翻译
    const response2 = await http.post('/stu/upload/translation', formData2.value, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    });
